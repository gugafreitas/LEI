            ...
            while (!socket.isInputShutdown()) {
                ...
                switch (opt) {
                    ...
                    case "reserva" -> {
                        ...
                        if (!ocupada) {
                            out.writeBoolean(true);
                            Reserva r = baseDados.getReservasBaseDados().adicionaReserva(idUtilizador, idTrotinete);
                            out.writeUTF(r.getIdReserva());

                            //List<Recompensa> lrAntes = rh.getLr();
                            barrier.notifica();
                            /*List<Recompensa> lrDepois = rh.getLr();
                            for(Recompensa recompensa : lrAntes){
                                out.writeBoolean(true);
                                recompensa.serialize(out);
                            }
                            out.writeBoolean(false);
                            for(Recompensa recompensa : lrDepois){
                                out.writeBoolean(true);
                                recompensa.serialize(out);
                            }
                            out.writeBoolean(false);*/
                    ...
                    case "alterarN" -> {
                        String utilizadorId = split[1];
                        String bool = split[2];
                        baseDados.getUtilizadoresBaseDados().getUtilizadorByID(utilizadorId).setNotificacao(Boolean.parseBoolean(bool));
                    }
                    case "alterarR" -> {
                        String utilizadorId = split[1];
                    ...
                    case "ativarN" -> {
                        ...
                        baseDados.getUtilizadoresBaseDados().getUtilizadorByID(utilizadorId).setNotificacao(true);
                        baseDados.getUtilizadoresBaseDados().getUtilizadorByID(utilizadorId).setRaio(d);
                    }
                    case "notificar" -> {
                        String utilizadorId = split[1];
                        while (in.readBoolean()){
                            if(baseDados.getUtilizadoresBaseDados().getUtilizadorByID(utilizadorId).isNotificacao())
